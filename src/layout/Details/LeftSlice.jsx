        ...
        <span className='flex gap-3'>
            {
                data?.genres &&
                data?.genres?.map(g =>
                    <span key={g.id} className={'rounded-3xl bg-indigo-700 px-3 max-w-[10em] overflow-hidden text-ellipsis'}>{g.name}</span>
                )
                ||
                ''
            }
...
const RateSection = ({ data, rated, ratedNow, handleRate }) => {
    return (
        ...
        <div className={`left-slice flex w-full h-full lg:min-h-[600px] lg:w-1/2 py-16 lg:px-20 px-10 text-white flex-col ${loading && 'place-content-center' || 'place-content-start'} place-items-start`}>
            {
                ...
                <>
                    ...
                    <RateSection data={data} rated={rated} ratedNow={ratedNow} handleRate={handleRate} />
